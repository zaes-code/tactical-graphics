...
import {buildTacticalGraphic, paintTacticalGraphic, carryPaintFlags, type MapLibreTacticalGraphic} from '../maplibreAdapter';
import {centerSymbolPlacement} from '../centerSymbol';

// Moved out so a renderer that is not MapLibre can place the symbol too; re-exported for the tests.
export {centerSymbolPlacement, followTaskCenter, securityOperationCenter} from '../centerSymbol';
import {
    GRAPHIC_ID_PROPERTY,
    bucketPaintsInto,
    handleLayer,
...
export class NativeLayerRenderer {
    ...
    private realizeCenterSymbols(visible: MapLibreTacticalGraphic[]): void {
        ...
        for (const graphic of visible) {
            ...
            const placed = centerSymbolPlacement(graphic, {resolution, measureText: this.measureText});
            if (!placed) continue;
