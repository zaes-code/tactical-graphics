import {followTaskSymbol} from '../symbology/followTaskPaints';
import {securityOperationSymbol} from '../symbology/securityPaints';
import type {PaintContext} from './paint';
import {CENTER_SYMBOL_GRAPHICS, escortSymbolSizePx} from './securitySymbol';
import {TacticalGraphicName} from './type';
import {lonLatToMercator as toMercator, mercatorToLonLat as toLonLat} from './mercator';
import type {PaintedGraphic} from './paintedGraphic';
...
export function followTaskCenter(
     ...
     * `buildPaintedGraphic` projects it on the way out and every paint function is handed
     * it in that frame — so running it through `projectToMercator` a second time read those
