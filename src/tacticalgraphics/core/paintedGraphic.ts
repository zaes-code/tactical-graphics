...
import {RSD_DEFAULT_START_SHARE} from '../graphics/MaritimeArea';
import {resolveRangeFanBands} from '../graphics/RangeFan';
import {withFittedDashes} from '../symbology/dashFit';
import {withHiddenAmplifiers} from '../symbology/paintFunctions';
import {getPaintFunction} from '../symbology/registry';
import {axisFromRectangleRing, RECTANGLE_DEFAULT_HALF_WIDTH_PX, rectangleDefaultHalfWidth} from './anchors';
import {arrowheadMeters, axisAndWidth, decorationMeters, drawnSizeMeters, hasAxisAndWidth, hasBakedDecoration} from './decorationSizes';
import {drawnAnchorFrame, drawnAnchors} from './drawnAnchors';
import {normalizeDrawnBase} from './drawnBase';
import {carriesRectangleLength, groundMeters, isMovementGraphic, isRectangular, RANGE_FANS, ratioLockOf, rectangleAmplifiers, shapedByWidth, usesDrawnAnchors} from './handles';
import {groundLength} from './mercator';
...
/**
 * # Generator output → paint features, for any renderer
 *
 * Written as the MapLibre twin of `openlayersAdapter.ts` and moved into the root once a
 * second renderer built from it: nothing here names a map library, and a renderer that
 * draws from the shared paint layer builds its graphics here so they agree on sizes,
 * defaults and old-file repairs. The `/maplibre` entry point still offers it under its old
 * names (`buildTacticalGraphic`, `MapLibreTacticalGraphic`).
 *
 * It is markedly shorter than the OpenLayers adapter, for a reason worth recording:
 * **the reprojection does not disappear, it moves.**
 *
 ...
 * meant `buildPaintedGraphic` refused them outright, so they were registered as
 ...
 */
export function projectToMercator(geometry: GeoJSONFeature['geometry']): ProjectedInputGeometry | undefined {
...
export interface PaintedGraphic {
    id: string;
 ...
 *
 * `buildPaintedGraphic` builds `graphic` and `labels` from `properties.tacticalGraphic`, so
 ...
 * or amplifier edit. Re-deriving them is what `buildPaintedGraphic` does at its top, so the
 * description is the honest input. @see describedProperties
 */
export function descriptionOf(graphic: PaintedGraphic): TacticalGraphicProperties {
...
export function carryPaintFlags(
    previous: PaintedGraphic,
    rebuilt: PaintedGraphic,
): PaintedGraphic {
...
function sizeDefaults(
     ...
     * `buildPaintedGraphic`. @see axisAndWidth, hasAxisAndWidth
     *
...
function ratioLockedSize(
    ...
    const projected = projectToMercator(baseGeometry);
...
export function buildPaintedGraphic(
    name: TacticalGraphicName,
    baseGeometry: GeoJSONFeature['geometry'],
    properties: Omit<TacticalGraphicProperties, 'name'> = {},
    drawingResolution?: number,
): PaintedGraphic | undefined {
    ...
    const graphicGeometry = projectToMercator(placed.graphic);
    if (!graphicGeometry) return undefined;

    const labelGeometry = projectToMercator(placed.labels);
    const center = projectToMercator(baseGeometry);
    ...
    const shared = {
        ...
        ring: outerRingOf(projectToMercator(baseGeometry)) ?? outerRingOf(graphicGeometry),
...
function handlePositions(geometry: GeoJSONFeature['geometry']): ProjectedPosition[] {
    const projected = projectToMercator(geometry);
    if (!projected) return [];
 ...
 */
export function paintGraphic(graphic: PaintedGraphic, context: PaintContext): Paint[] {
